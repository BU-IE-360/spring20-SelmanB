...
### Predictor Filling  
In order to avoid bias due to having data of the current day for the training data and two days prior for prediction, estimating today’s sales numbers from the data of the two days before was another idea. xdata_lag list was created for that purpose. To be used before predicting regressors, from the data of two days before, sold_count data of the today was tried to predict.
```{r, warning=FALSE , message=FALSE}
xdata_lag <- list()
for(i in 1:8){
...
Qualitative inspection of partial and conventional autocorrelation functions exhibited possibility of ARIMA modelling, especially with weekly seasonality. In order to incorporate ARIMA terms in the combined model, lagged regressors are incorporated to the data. Merging of these AR regressors and other regressors was accomplished not by averaging or combining their results but building a model that is optimized including all these regressors. This has the advantage of being able to capture more correlations in exchange of reduced robustness. Therefore no library functions for ARIMA modelling is not used.

So this approach was for the last 22 days. sold_count data of 7 days before, 14 days before and 21 days before were added for each product everyday. This step was applied both to xdata and xdata_lag. If effectiveness of that approach is not satisfying, in the loop which was written to create a linear model, it was not going to be used.Output of the part until this point and output of the continuing part were compared and according to observations more dependable one submitted.
```{r,  warning=FALSE , message=FALSE}
...
### Modelling 
Automated selection of regressors was pursued before building a hybrid regression model containing AR components. Initially, a linear model including all possible regressors was calculated and only the regressors having a p-value greater than a threshold were chosen. This required only O(1) models to be built. The next approach was to eliminate the unsignificant regressors one by one as per the common stepwise scenario, building O(n) models in total. This was found to be computationally feasible and subsequently employed as described below. Other more precise approaches with computational complexities greater than linear were discussed and found to be realistically infeasible.
